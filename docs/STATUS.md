...
**Stand:** 01.08.2026<br>
**Phase:** M0 — Foundation abgeschlossen (29.07.2026, formale Schließung erfolgt); M1 — Klassen abgeschlossen (31.07.2026, formale Schließung erfolgt); M2 — Raumeditor abgeschlossen (31.07.2026, formale Schließung erfolgt); M3 — Persistente Pläne ist die aktuelle Frontier, Child-Slices #56, #57 und #58 umgesetzt<br>
**Produktcode:** Auth-Grundlage (M1 #42), Klassenverwaltung (M1 #43), Schülerprofile und Sitzregeln (M1 #44), persistenter Foto-Upload (M1 #45), CSV-Import für Klassenlisten (M1 #46), Raumvorlagen-Grundlage (M2 #49), Raum-Canvas (M2 #50), Standardobjekte mit Möbelpalette (M2 #51), Objektinteraktion mit Rasterfang (M2 #52), Objektaktionen (M2 #53), adressierbare Sitzplätze (M2 #54), die M2-Akzeptanz (M2 #55), die Sitzplan-Grundlage (M3 #56), die Schülerzuordnung (M3 #57) und die Editor-Historie (M3 #58) vorhanden
...
- aktualisierte Werkzeug- und Migrationsbasis: Next.js 16.2.12 innerhalb der LTS-Range `~16.2.11` ([#39](https://github.com/arn0ld87/abschlussprojekt_beziehung/issues/39), PR [#141](https://github.com/arn0ld87/abschlussprojekt_beziehung/pull/141)), drizzle-kit 0.31 ([#143](https://github.com/arn0ld87/abschlussprojekt_beziehung/issues/143), PR [#147](https://github.com/arn0ld87/abschlussprojekt_beziehung/pull/147)) sowie lückenlose drizzle-Snapshot-Kette ohne Schema-Drift mit idempotenter Migration `0007_constraint_namen_angleichen.sql` und Journal-Test ([#148](https://github.com/arn0ld87/abschlussprojekt_beziehung/issues/148), PR [#149](https://github.com/arn0ld87/abschlussprojekt_beziehung/pull/149))
- Schülerzuordnung: normalisierte, deterministisch nach `sitzplatzId` sortierte Zuordnungsliste im bestehenden jsonb-`canvasDocument` (keine Migration nötig) mit vier harten, per `superRefine` erzwungenen Invarianten (nur Sitzplätze der eingefrorenen Geometrie, höchstens ein Schüler je Sitzplatz, höchstens ein Sitzplatz je Schüler, aufsteigend nach `sitzplatzId` sortiert), framework-freie Commands `setzeSchueler`/`tausche`/`entferne` ohne React-, Konva- oder Drizzle-Bezug, serverseitig erzwungene Klassenzugehörigkeit und Soft-Delete-Prüfung im `SitzplanService` für jede neu gesetzte Zuordnung (bereits im Dokument stehende, inzwischen inaktive Einträge bleiben zulässig und werden als Befund gemeldet, damit sich ein Altbestand schrittweise aufräumen lässt), Route Handler `PUT /api/sitzplaene/[id]/zuordnungen` (schreibt das vollständige validierte Dokument, noch ohne Debounce-Autosave), abgeleitete Ablage samt Inkonsistenzbefund für nicht mehr aktive Schülerprofile (Befund blockiert das Laden nicht) sowie Editor-UI mit Konva-Sitzplandarstellung, Drag-and-drop in genau den vier spezifizierten Richtungen und gleichwertiger Tastaturbedienung über Auswahl und Aktion (die Bedienentscheidungen liegen im framework-freien `zuordnung-interaktion.ts` und sind dort als Verhalten getestet; „Ablage → belegter Platz“ wird begründet abgelehnt statt zu verdrängen); Eindeutigkeit und Referenzintegrität sind über Property-Tests nach beliebigen Command-Folgen belegt (M3 #57)
- Editor-Historie: framework-freies Modul `src/domain/sitzplan/historie.ts` mit unveränderlichem Zustandsstapel (Vergangenheit, Gegenwart, Zukunft, zuletzt bestätigter Serverstand) ohne React-, Konva- oder Drizzle-Bezug; die Historie hält vollständige Zuordnungslisten als Clientzustand, kein Ereignisprotokoll, und wird nie gesendet oder gespeichert (ADR-0010); Undo/Redo für Setzen, Tauschen und Entfernen, Verwerfen des Wiederherstellungszweigs nach einer neuen Änderung, Begrenzung auf 50 Schritte unter Erhalt der Gegenwart, Rücksetzen beim Laden eines anderen Plans; sichtbarer Änderungszustand `gespeichert | geändert | speichert | fehler` aus dem Vergleich gegen das zuletzt vom Server bestätigte Dokument (eine Bestätigung verschiebt nur die Vergleichsbasis und verbraucht kein Undo); Toolbar-Schaltflächen und Tastaturkürzel mit Plattformkonvention (Meta auf macOS, Strg sonst, Umschalt beziehungsweise Strg+Y für Wiederherstellen), die im Namensfeld des Plans und in jeder anderen Texteingabe bewusst nicht greifen — Kürzelentscheidung und Historienbedienung liegen im framework-freien `historie-bedienung.ts` und sind dort als Verhalten getestet (M3 #58)

## Noch nicht vorhanden

- Autosave mit Debounce und Revisionskonflikten, Planversionen und Restore im Sitzplan-Editor
- Release-Artefakt
...
| M3 | [#5 Persistente Pläne: Autosave, Versionen und Restore](https://github.com/arn0ld87/abschlussprojekt_beziehung/issues/5) | aktuelle Frontier — Child-Issues [#56](https://github.com/arn0ld87/abschlussprojekt_beziehung/issues/56) (Sitzplan-Grundlage), [#57](https://github.com/arn0ld87/abschlussprojekt_beziehung/issues/57) (Schülerzuordnung) und [#58](https://github.com/arn0ld87/abschlussprojekt_beziehung/issues/58) (Editor-Historie) umgesetzt |

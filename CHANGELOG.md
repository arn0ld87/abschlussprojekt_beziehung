...
- Schülerzuordnung (M3 #57): normalisierte Zuordnungsliste `{ sitzplatzId, schuelerId }[]` im bestehenden jsonb-`canvasDocument` (keine Migration nötig, `SitzplanDokumentV1` bleibt Version 1) statt des bisherigen leeren Platzhalters — bewusst eine Liste statt `Record<SitzplatzId, SchuelerId>`, weil sie zur Array-Form der Geometrie passt, JSON-stabil ist und stabil nach `sitzplatzId` sortiert deterministisch serialisiert; drei harte, per `superRefine` am Dokumentvertrag erzwungene Invarianten (jede `sitzplatzId` existiert in der eingefrorenen `raumGeometrie.sitzplaetze`, höchstens ein Schüler je Sitzplatz, höchstens ein Sitzplatz je Schüler); framework-freies Command-Modul `src/domain/sitzplan/zuordnung-commands.ts` mit `setzeSchueler` (setzt um statt zu duplizieren), `tausche` (atomar, auch bei nur einem belegten Platz) und `entferne` — alle ohne Mutation der Eingabe und ohne React-, Konva- oder Drizzle-Bezug; `SitzplanService.setzeZuordnungen` erzwingt serverseitig Eigentümerschaft am Plan sowie Klassenzugehörigkeit und Soft-Delete-Zustand jedes neu zugeordneten Schülerprofils (bereits im Dokument stehende, inzwischen nicht mehr aktive Einträge bleiben zulässig, damit sich ein Altbestand schrittweise aufräumen lässt) und schreibt das vollständige erneut validierte Dokument (Revision bleibt unverändert, Debounce-Autosave folgt mit M3 #59, ADR-0004); `SitzplanService.ansicht` liefert Plan, abgeleitete Ablage, Belegung und Inkonsistenzbefunde in einem Zug — die Ablage ist bewusst kein persistiertes Feld, sondern die Differenz aus aktiven Schülerprofilen und Zuordnungen, und ein Befund über ein nicht mehr aktives Schülerprofil verhindert das Laden des Plans nie; Route Handler `PUT /api/sitzplaene/[id]/zuordnungen`; Editor-UI `/sitzplaene/[id]` mit Konva-Sitzplandarstellung (belegte Plätze in Schülerfarbe mit Initialen, freie Plätze ungefüllt, keine persistierten Konva-Knoten, ADR-0002), Drag-and-drop in allen vier Richtungen (Ablage → Platz, Platz → Platz, Platz → belegter Platz als definierter Tausch, Platz → Ablage) und gleichwertiger, tastaturbedienbarer Alternative über Auswahl und Aktion mit `aria-pressed`, `aria-label`, sichtbarem Fokus und Live-Region; Property-Tests belegen Eindeutigkeit und Referenzintegrität nach beliebigen Command-Folgen (seed-gesteuert und reproduzierbar), der PostgreSQL-Integrationstest den Akzeptanzpfad Setzen → zweiter Schüler → Tausch → Zurücklegen → Neuladen
